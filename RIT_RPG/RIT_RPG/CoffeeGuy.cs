...
namespace RIT_RPG
{
    class CoffeeGuy:Character //Handles files for the Coffee Guy
    {
        public CoffeeGuy(SpriteBatch sb, GraphicsDeviceManager gd, Texture2D sprite, string fileName):base(sb, gd, sprite, fileName) //Constructor
        {

        }

        public CoffeeGuy(Character ch):base(ch) //Cloning constructor
        {
        ...
        public override void ExpGain(int expGained) //Handles exp gain
        {
            if (level < 100) //While the character's level is less than 100
            {
                ...
                {
                    StreamWriter fileWriter = new StreamWriter("coffeeGuy.bin"); //Create the StreamWriter object
