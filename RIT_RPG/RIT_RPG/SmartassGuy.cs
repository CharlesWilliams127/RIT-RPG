...
namespace RIT_RPG
{
    class SmartassGuy:Character //Handles files for the Smartass Guy
    {
        public SmartassGuy(SpriteBatch sb, GraphicsDeviceManager gd, Texture2D sprite, string fileName):base(sb, gd, sprite, fileName) //Constructor
        {

        }

        public SmartassGuy(Character ch):base(ch) //Constructor for cloning
        {
        ...
        public override void ExpGain(int expGained) //Handles exp gain
        {
            if (level < 100) //While the character's level is less than 100
            {
                ...
                {
                    StreamWriter fileWriter = new StreamWriter("smartassGuy.bin"); //Create the StreamWriter object
