...
namespace RIT_RPG
{
    class WittyGuy:Character //Handles files for the Witty Guy
    {
        public WittyGuy(SpriteBatch sb, GraphicsDeviceManager gd, Texture2D sprite, string fileName):base(sb, gd, sprite, fileName) //Constructor
        {

        }

        public WittyGuy(Character ch):base(ch) //Cloning constructor
        {
        ...
        public override void ExpGain(int expGained) //Handles exp gain
        {
            if (level < 100) //While the character's level is less than 100
            {
                ...
                {
                    StreamWriter fileWriter = new StreamWriter("wittyGuy.bin"); //Create the StreamWriter object
